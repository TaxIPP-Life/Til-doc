...
Il y n'y a guère que deux options possibles. 
La première consiste, lors du matching, si on associe les 5 000 personnes aux 10 000, séparer les 10 000 en deux  groupes de 5 000 (représenter par deux individus dans la base) et marier un des groupes laissant l'autre célibataire. 
La seconde est de dupliquer dès le départ les individus pour avoir une pondération de 1 et autant de lignes que l'on a d'individu dans la population étudiée.
...
Il faut en effet garder à l'esprit que si un individu est "splité" alors il faut aussi spliter toutes les personnes qui lui sont liées : ses enfants par exemple. 
Après les enfants, et les parents, les conjoints de ceux-ci, puis les parents et enfants des conjoints, etc.
Une sorte de théorème des 6 poignées de main (pas exactement lui puisqu'on ne regarde que le lien familiaux ici) laisse sous-entendre qu'au bout de quelques simulations de mariage, la population a la taille de la pondération minimale. 

Il est plus simple pour l'instant de ne pas trop s'éloigner de Liam2. 
De plus, et c'est un point important, la duplication de l'échantillon a l'avantage de permettre au même individu de la base initiale d'avoir plusieurs trajectoires professionnelles par exemple.
...
Dupliquer les ménages ou les individus en fonction du poids n'est pas en soi compliqué.
En revanche, il faut être minutieux sur le lien entre les différentes lignes des tables. 
...
Pourquoi cela ? 
Il faut savoir que dans l'enquête Patrimoine 2010, qui contient un sur-échantillonnage des hauts patrimoines, la plus petite pondération est de 6.
...
Si on effectue cette étape après la fermeture de l'échantillon (ce qui est souhaitable pour optimiser les chances d'avoir des liens parents-enfants représentatifs), il faut aller chercher les ménages rattachés. 
Sinon, il faut effectuer ensuite l'étape de fermeture.
On fait ainsi une sorte de bootstrap tout en travaillant avec une pondération uniforme, ce qui peut être intéressant.
...
Par rapport aux autres étapes, la formation des unions a ceci de particulier qu'il s'agit d'un marché et non pas d'une imputation individuelle. 
...
Elle a aussi le désavantage d'être avant tout pensée pour des matrices carrées, avec autant de candidats des deux côtés du mariage. 
